...
class TypeInfoEntry:
    ...
    def _normalize_loc(self, loc):
        if loc[0].startswith("fbreg"):
            ...
            # fbreg(rsp) --> RSP
            normalized_reg_str = (
                loc[0].replace("fbreg", "").replace("(", "").replace(")", "")
            )
    ...
    def __repr__(self) -> str:
        return "TIE-[%x, %x)@%s: %s" % (self.low_pc, self.high_pc, self.loc, self.type)
...
def _build_tree(sorted_points, ties, parent):
    ...
    median_ties = [
        tie for tie in ties if tie.low_pc <= median and tie.high_pc >= median
    ]
