...
def parse_args():
    ...
    parser.add_argument(
        "--raw-data-in",
        type=str,
        help="path to the .pkl file corresponding to the jsonl data",
    )
    parser.add_argument(
        "--bin-dir-root", type=str, help="root dir containing the binaries"
    )
    ...
    for line in tqdm(jsonl_data_in):
        ...
        if (project_name, binary_name, function_addr) not in name2raw:
            raise Exception(
                ...
                % str((project_name, binary_name, function_addr))
            )
        ...
        if (project_name, normalized_bin_name) not in binname2debug_info:
            ...
            bin_path = os.path.join(
                args.bin_dir_root,
                project_name,
                normalized_bin_name + ".type_info.jsonl",
            )
        ...
        current_dbg_info_searcher = binname2debug_info[
            (project_name, normalized_bin_name)
        ]
