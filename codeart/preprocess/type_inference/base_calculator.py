...
def solve_type(die):
    ...
    if tag == "DW_TAG_base_type":
        ...
        if "DW_AT_type" not in die.attributes:
            return "void"
        base = solve_type(
            die_globals.all_dies[resolve_ofs(die.attributes["DW_AT_type"].value)]
        )
        return base
    ...
    if tag == "DW_TAG_base_type":
        ...
        for child in die.iter_children():
            if child.tag == "DW_TAG_member":
                ...
                type_tag = die_globals.all_dies[
                    resolve_ofs(child.attributes["DW_AT_type"].value)
                ]
...
def calculate_base_addr(die, sub_prog_range_list):
    ...
    if "DW_AT_abstract_origin" in die.attributes:
        ...
        type_entry = die_globals.all_dies[
            resolve_ofs(original_def.attributes["DW_AT_type"].value)
        ]
        ...
        type_entry = die_globals.all_dies[
            resolve_ofs(die.attributes["DW_AT_type"].value)
        ]
        type_info = solve_type(type_entry)
