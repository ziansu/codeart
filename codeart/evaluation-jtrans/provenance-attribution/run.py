...
from models import (
    JTransModel, JTransForSequenceClassification
)
...
class DataTrainingArguments:
    ...
    label2id_file: str = field(default='label2id.json', metadata={"help": "Dict mapping label to its id."})
...
def main():
    ...
    # l# process labels
    label_list = model_args.labels.split(',')
    label2id = {l: i for i, l in enumerate(label_list)}
    id2label = {i: l for i, l in enumerate(label_list)}
    num_labels = len(label_list)
    
    jtrans_model = JTransModel.from_pretrained(model_args.model_name_or_path)
    jtrans_config = jtrans_model.config
    jtrans_config.problem_type = 'single_label_classification'
    jtrans_config.num_labels = num_labels
    jtrans_config.label2id = label2id
    jtrans_config.id2label = id2label

    tokenizer = BertTokenizer.from_pretrained(
        model_args.model_name_or_path
    )
    model = JTransForSequenceClassification.from_pretrained(
        model_args.model_name_or_path,
        config=jtrans_config
    )

    def preprocess_function(example):
        # example_entry = json.loads(example)
        example_entry = example
        funcstr = example_entry['jtrans_function_string']
        # Tokenize the texts
        ...
        tokenizer_ret["label"] = label2id[eval(example_entry['metadata'])['from']]
        return tokenizer_ret
    
    column_names = ['metadata', 'jtrans_function_string']

    with training_args.main_process_first(desc="dataset map tokenization"):
        # debug
        raw_datasets = raw_datasets.shuffle(seed=42)  # test if possible with only the train part
